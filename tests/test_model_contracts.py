...
SPEC = importlib.util.spec_from_file_location("run_r2", ROOT / "scripts/rebuild_models.py")
...
def test_committed_cre_shock_audit_exactly_reconstructs_interaction_input():
    audit_path = ROOT / "results/evidence/cre_price_shock.csv"
    audit = pd.read_csv(
        audit_path,
    ...
    metadata = next(row["original_lineage"] for row in provenance["files"]
                    if row["file"] == "evidence/cre_interaction.csv")
