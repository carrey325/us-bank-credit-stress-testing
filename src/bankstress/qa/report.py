...
def write_qa(standard: pd.DataFrame, panel: pd.DataFrame, mapping: pd.DataFrame, qa_dir: Path, nco_exceptions: pd.DataFrame | None = None) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    panel = panel.copy()
    for column in ["tier1_capital", "risk_weighted_assets", "tier1_ratio", "computed_tier1_ratio", "equity_capital", "equity_to_assets_ratio"]:
    ...
    controls = panel.groupby(["bank_id", "report_date"], as_index=False).agg(
        ...
        total_recovery=("total_recovery", "first"), tier1_capital=("tier1_capital", "first"),
        risk_weighted_assets=("risk_weighted_assets", "first"), tier1_ratio=("tier1_ratio", "first"),
        computed_tier1_ratio=("computed_tier1_ratio", "first"),
    ...
    recon["capital_ratio_difference_basis_points"] = (recon["computed_tier1_ratio"] - recon["tier1_ratio"]) * 10_000
    recon["capital_reconciliation_tolerance_basis_points"] = CAPITAL_RATIO_TOLERANCE_BASIS_POINTS
    capital_evaluable = recon[["tier1_capital", "risk_weighted_assets", "tier1_ratio", "computed_tier1_ratio"]].notna().all(axis=1)
    capital_within_tolerance = recon["capital_ratio_difference_basis_points"].abs().le(CAPITAL_RATIO_TOLERANCE_BASIS_POINTS)
    recon["capital_reconciliation_status"] = "NOT_EVALUABLE_MISSING_INPUT"
    recon.loc[capital_evaluable & capital_within_tolerance, "capital_reconciliation_status"] = "PASS_WITHIN_TOLERANCE"
    recon.loc[capital_evaluable & ~capital_within_tolerance, "capital_reconciliation_status"] = "FAIL_OUTSIDE_TOLERANCE"
    ...
    capital_statuses = recon["capital_reconciliation_status"].value_counts().to_dict()
    capital_evaluable_count = int(capital_evaluable.sum())
    capital_pass_count = int((recon["capital_reconciliation_status"] == "PASS_WITHIN_TOLERANCE").sum())
    capital_coverage = capital_evaluable_count / len(recon) if len(recon) else 0.0
    capital_pass_rate = capital_pass_count / capital_evaluable_count if capital_evaluable_count else 0.0
    component_columns = [column for column in ["charge_off_component_complete", "recovery_component_complete"] if column in panel]
    incomplete_component_flows = int(sum(panel[column].eq(0).sum() for column in component_columns))
    report = "\n".join([
        "# Batch 1 data-quality report", "", f"- Standard observations: {len(standard):,}", f"- Derived observations: {len(panel):,}", f"- Core banks: {panel.bank_id.nunique():,}", f"- Field-mapping rows: {len(mapping):,}", f"- Duplicate standard bank/date/raw-code keys: {int(standard.duplicated(['bank_id', 'report_date', 'raw_code']).sum())}", f"- Negative NCO observations retained: {int((panel.segment_nco < 0).sum())}", f"- Suppressed incomplete segment gross flows: {incomplete_component_flows:,}", f"- FDIC merger quarters flagged: {int(panel.merger_quarter_flag.sum())}", f"- Asset-jump quarters flagged: {int(panel.asset_jump_flag.sum())}", f"- Manual formula-audit failures: {int((audit.pass_fail == 'fail').sum())}", "", "## Definitions", "", "- CRE uses reported aggregate RIAD3582/3583 and RIAD3590/3591 through 2007Q4. From 2008Q1, construction is RIADC891+C893 / C892+C894 and nonfarm nonresidential is RIADC895+C897 / C896+C898; multifamily RIAD3588/3589 remains continuous.", "- A segment gross flow is emitted only when every required component is present and quarterized; incomplete component sets remain missing rather than becoming partial flows.", "- Mortgage is closed-end 1-4 family residential lending only: RC-C RCON/RCFD5367 + 5368 and RI-B RIADC234 + C235 - C217 - C218. Revolving/open-end RCON/RCFD1797 is excluded.", "- Segment NPL is unavailable in a stable mapping. `bank_total_npl`, `bank_total_npl_ratio`, and their lags are bank-level fallback controls; `segment_npl_rate` is deliberately missing rather than total NPL divided by segment exposure.", "- `tier1_capital` and reported `tier1_ratio` use Schedule RC-R fields; `computed_tier1_ratio` is Tier 1 capital divided by mapped RWA. `equity_capital` and `equity_to_assets_ratio` are separately named book-equity measures.", "- `allowance_coverage` equals allowance / bank total noncurrent loans (also named `allowance_to_total_npl`), not allowance / total loans.", "", "## NCO reconciliation rule", "", "- CRE, C&I, and mortgage are mapped loan segments, not the entire Call Report loan portfolio; their net NCO is therefore not required to equal reported total NCO.", f"- For bank-quarters with all four gross-flow values, mapped charge-offs and recoveries must each be no more than reported total plus {NCO_GROSS_FLOW_TOLERANCE_THOUSANDS:,.0f} thousand dollars.", "- A downward YTD revision explains only the corresponding gross-flow excess when the reported total for that same flow is revised downward. Unrelated or mapped-component revisions remain REVIEW_REQUIRED.", "- A directly verified source-filing inconsistency may be explained only when it is listed in `metadata/nco_reconciliation_exceptions.csv`; it remains visible in the reconciliation output.", *[f"- {status}: {count:,}" for status, count in sorted(nco_statuses.items())], f"- Open review items: {review_count:,}; see `metadata/nco_reconciliation_review.md`.", "", "## Capital reconciliation", "", "- Formula: mapped Tier 1 capital / mapped risk-weighted assets versus the separately reported Tier 1 risk-based capital ratio.", f"- Absolute tolerance: {CAPITAL_RATIO_TOLERANCE_BASIS_POINTS:g} basis point; ratios carrying a `%` suffix in the CDR files are normalized to decimal fractions before comparison.", f"- Evaluable bank-quarters: {capital_evaluable_count:,}/{len(recon):,} ({capital_coverage:.2%}).", f"- Within tolerance: {capital_pass_count:,}/{capital_evaluable_count:,} ({capital_pass_rate:.2%}) when evaluable.", *[f"- {status}: {count:,}" for status, count in sorted(capital_statuses.items())], "", "## Mapped raw-field presence", "", *[f"- {date}: {count} mapped raw codes" for date, count in mapping_validation.items()], "", "## Limitations", "", "- The historical Call Report taxonomy has genuine reporting-detail changes. Missing segment detail remains missing; it is never filled with zero.", "- FDIC history events are branch-granular and collapsed to bank-quarter merger flags; no virtual-bank reconstruction is claimed.", "- `metadata/manual_source_audit.csv` records the independent raw-archive source-document sample; the deterministic audit is retained as a separate formula control.", ""
    ])
