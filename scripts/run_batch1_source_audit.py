...
def _trace_metric(
    ...
    return float(current["quarterly_value"].sum(min_count=1)), field_values, sorted(set(members)), all(archive_matches)
...
def create_source_audit(sample_size: int = 100, random_state: int = 772) -> pd.DataFrame:
    ...
    source = panel.dropna(subset=["charge_off", "recovery", "segment_nco"]).sample(sample_size, random_state=random_state).copy()
    ...
    for observation in source.itertuples(index=False):
        ...
        charge_off, co_fields, co_members, co_archive_match = _trace_metric(coalesced, raw_dir, observation.bank_id, date, observation.segment, "charge_off")
        recovery, rec_fields, rec_members, rec_archive_match = _trace_metric(coalesced, raw_dir, observation.bank_id, date, observation.segment, "recovery")
        ...
        rows.append({
            ...
            "limitation": "This is a fixed random 100-observation source-document sample, not evidence that all unreviewed reporting-detail changes are absent.",
        })
